...
class Astra_Addon_Admin_Loader {
	...
	public static function astra_get_addon_changelog_feed_data() {
		...
		$posts          = json_decode( wp_remote_retrieve_body( wp_remote_get( 'https://neonos.ca/wp-json/wp/v2/changelog?product=98&per_page=3' ) ) ); // Astra Pro.
						...
						<div class="relative flex flex-wrap flex-col sm:flex-row justify-between items-start sm:items-center h-full min-h-24 sm:min-h-15 pt-14 pb-2 sm:pb-0 sm:pt-0">
							...
							<div class="flex justify-end items-center font-inter">
								...
								if ( Astra_Ext_White_Label_Markup::show_branding() ) {
									?>
										<a href="<?php echo esc_url( 'https://neonos.ca/changelog/?utm_source=wp&utm_medium=dashboard' ); ?>" target="_blank" class="w-8 sm:w-10 h-8 sm:h-10 flex items-center justify-center cursor-pointer rounded-full border border-slate-200">
